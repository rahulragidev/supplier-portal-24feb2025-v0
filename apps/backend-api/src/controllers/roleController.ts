...
import { NewEmployeeOrgUnitRoleSchema, NewRoleSchema } from "@workspace/database/zod-schema";
...
export const roleController = {
  ...
  async getAllRoles(c: Context) {
    try {
      const allRoles = await db.select().from(role).where(isNull(role.deletedAt));
  ...
  async getRolesByOrganization(c: Context) {
    try {
      ...
      const roleData = await db
        .select()
        .from(role)
        .where(and(eq(role.organizationUid, orgUid), isNull(role.deletedAt)));
  ...
  async getRoleById(c: Context) {
    try {
      ...
      const roleData = await db
        .select()
        .from(role)
        .where(and(eq(role.uid, uid), isNull(role.deletedAt)));
  ...
  async assignEmployeeRole(c: Context) {
    try {
      ...
      const existingUser = await db.select().from(appUser).where(eq(appUser.uid, employeeUserUid));
  ...
  async removeEmployeeRole(c: Context) {
    try {
      ...
      const updated = await db
        ...
        .where(and(eq(employeeOrgUnitRole.uid, uid), isNull(employeeOrgUnitRole.deletedAt)))
        .returning();
